...
public class WorkDayUtils {
	...
	public static void main(String[] args) {
		try {
			...
			System.out.println("�?始日�?" + cal_start.get(Calendar.YEAR) + "-" + (cal_start.get(Calendar.MONTH) + 1) 
	...
	public int getWorkingDay(java.util.Calendar d1, java.util.Calendar d2) {
		...
		// int charge_date = 0;
		int charge_start_date = 0;// �?始日期的日期偏移�?
		...
		if (stmp != 0 && stmp != 6) {// �?始日期为星期六和星期日时偏移量为0
			charge_start_date = stmp - 1;
		}
		if (etmp != 0 && etmp != 6) {// 结束日期为星期六和星期日时偏移量�?0
	...
	public String getChineseWeek(Calendar date) {
		final String dayNames[] = { "星期�?", "星期�?", "星期�?", "星期�?", "星期�?", "星期�?", "星期�?" };
